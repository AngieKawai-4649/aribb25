...
static void test_arib_std_b25(
#ifndef ENABLE_ARIB_STREAM_TEST
	const TCHAR* src, const TCHAR* dst,
#endif
	OPTION* opt);
...
#endif
{
	...
	*/
	...
	ARIB_STD_B25_BUFFER sbuf = {NULL,0};
...
#else
#ifdef _WIN32
	int rtn = _tsopen_s(&sfd, src, _O_BINARY | _O_RDONLY | _O_SEQUENTIAL, _SH_DENYWR, _S_IREAD);
	if (rtn != 0) {
		_ftprintf(stderr, _T("error - failed on _sopen_s(%s) [src]\n"), src);
		goto LAST;
	}
#else
...
#ifndef ENABLE_ARIB_STREAM_TEST
#ifdef _WIN32
	rtn = _tsopen_s(&dfd, dst, _O_BINARY | _O_WRONLY | _O_SEQUENTIAL | _O_CREAT | _O_TRUNC, _SH_DENYRW, _S_IWRITE);
	if (rtn != 0) {
		_ftprintf(stderr, _T("error - failed on _sopen_s(%s) [dst]\n"), dst);
		goto LAST;
	}
#else
		...
		if(code < 0){
			...
			if(code < ARIB_STD_B25_ERROR_NO_ECM_IN_HEAD_32M){
				...
				if(sbuf.size > 0){
					...
					p = (uint8_t *)malloc((size_t)(sbuf.size) + n);
				}
				if(p != NULL){
