...
/****************************************************************************/
typedef struct {
	int64_t            card_id;					/* 6byte:descriptor[0-5]   カードID				*/
	int32_t            message_length;			/* 2byte:descriptor[6-7]   メッセージバイト長	*/
	int32_t            protocol_number;			/* 1byte:descriptor[8]     プロトコル番号		*/
	int32_t            broadcaster_group_id;	/* 1byte:descriptor[9]     事業体識別			*/
	int32_t            message_id;				/* 1byte:descriptor[10]    メッセージID			*/
	int32_t            message_control;			/* 1byte:descriptor[11]    メッセージ制御		*/
												/* Nbyte:descriptor[12-]   メッセージコード領域 */
												/* N=メッセージバイト長-4  4=プロトコル番号から	*/
												/* メッセージ制御までの4byte					*/
...
/* EMG 処理追加 2024/5/24													*/
/****************************************************************************/
static void extract_emg_fixed_part(EMG_FIXED_PART* dst, uint8_t* src);
/****************************************************************************/
...
static int put_arib_std_b25(void *std_b25, ARIB_STD_B25_BUFFER *buf)
{
	...
	if(prv->p_count < 1){
		...
		if(prv->p_count < 1){
			if(prv->sbuf_offset < (intptr_t)((16*1024*1024))){
				/* need more data */
	...
	if(!check_pmt_complete(prv)){
		...
		if(!check_pmt_complete(prv)){
			if(prv->sbuf_offset < (intptr_t)((32*1024*1024))){
				/* need more data */
...
static int select_unit_size(ARIB_STD_B25_PRIVATE_DATA *prv)
{
	...
	if((tail-head) < (uintptr_t)(188*16)){
		// need more data
		return 0;
	}
	if((tail-head) > (uintptr_t)(188*32)){
		tail = head + (188*32);
	}
	...
	n = 0;
	for(i=188;i<320;i++){
		if(m < (intptr_t)(count[i-188])){
			m = (intptr_t)(count[i-188]);
			n = i;
		}
	}
	...
	w = m*n;
	if( (m < 8) || ((w+(intptr_t)(3*n)) < (tail-head)) ){
		return ARIB_STD_B25_ERROR_NON_TS_INPUT_STREAM;
	}
...
static int proc_emm(ARIB_STD_B25_PRIVATE_DATA *prv)
{
	...
	while( (n = prv->emm->get_count(prv->emm)) > 0 ){
		...
		if(sect.hdr.table_id == TS_SECTION_ID_EMM_MESSAGE){
			/o EMM_MESSAGE is not supported o/
			goto NEXT;
...
static int proc_emm(ARIB_STD_B25_PRIVATE_DATA* prv)
{
	...
	while ((n = prv->emm->get_count(prv->emm)) > 0) {
		...
		else if (sect.hdr.table_id == TS_SECTION_ID_EMM_MESSAGE) {
			while ((head + 13) <= tail) {

				extract_emg_fixed_part(&emg_hdr, head);
				len = emg_hdr.message_length + 8; /* 5 = cardID 6byte message_length 2byte  */
				if ((head + len) > tail) {
					/* broken EMG element */
					goto NEXT;
				}
				...
				for (j = 0; j < prv->casid.count; j++) {
					if (prv->casid.data[j] == emg_hdr.card_id) {
						n = prv->bcas->proc_emg(prv->bcas, head, len);
						if (n < 0) {
							r = ARIB_STD_B25_ERROR_EMG_PROC_FAILURE;
							goto LAST;
						}
						unlock_all_decryptor(prv);
					}
...
static void extract_emg_fixed_part(EMG_FIXED_PART* dst, uint8_t* src)
{
	...
	dst->message_length = (src[6] << 8) | src[7];
	dst->protocol_number = src[8];
	dst->broadcaster_group_id = src[9];
	dst->message_id = src[10];
	dst->message_control = src[11];
}
...
#if defined(_WIN32)
	*time += (int64_t)(timeGetTime() - start_time);
#else
