...
#if defined(_WIN32)
	// dllで動作している時はlibaribb25.cpp BOOL WINAPI DllMain() でインスタンスをセット
	HINSTANCE h_module = NULL;
#endif
...
/* EMG 処理追加 2024/5/24													*/
/****************************************************************************/
static const uint8_t EMG_RECEIVE_CMD_HEADER[] = {
#ifdef ENABLE_ARIB_STD_B1
	0x80, 0x38, 0x00, 0x00,
#else
	0x90, 0x38, 0x00, 0x00,
...
/* EMG 処理追加 2024/5/24													*/
/****************************************************************************/
static int proc_emg_b_cas_card(void* bcas, uint8_t* src, int len);
/****************************************************************************/
...
/* EMG 処理追加 2024/5/24													*/
/****************************************************************************/
	r->proc_emg = proc_emg_b_cas_card;
/****************************************************************************/
...
/* EMG 処理追加 2024/5/24													*/
/****************************************************************************/
static int setup_emg_receive_command(uint8_t* dst, uint8_t* src, int len);
/****************************************************************************/
...
#ifdef _WIN32
static void get_ini_cardreader(LPTSTR cardreadername, const unsigned long len)
{
	// dll動作時はdllのパス、exe動作時はexeのパスを取得
	TCHAR ini_file_path[MAX_PATH];
	GetModuleFileName(h_module, ini_file_path, MAX_PATH);
	...
	_tsplitpath_s(ini_file_path, szDrive, _MAX_DRIVE, szDir, _MAX_DIR, szFname, _MAX_FNAME, szExt, _MAX_EXT);
	...
	GetPrivateProfileString(_T("CardReader"), _T("Name"), NULL, cardreadername, (DWORD)len - 1, ini_file_path);
...
static int init_b_cas_card(void* bcas)
{
	size_t m;
	long ret;
	unsigned long len = 0, alloc_len = 0;
	...
	TCHAR ini_name[1024] = { 0, };
	unsigned long namelen;
#ifdef _WIN32
	get_ini_cardreader(ini_name, (unsigned long)(sizeof(ini_name) / sizeof(TCHAR)));
	namelen = (unsigned long)_tcsnlen(ini_name, sizeof(ini_name) / sizeof(TCHAR));
#endif

	LPTSTR rp = NULL, readers = NULL;
	len = SCARD_AUTOALLOCATE;
	ret = SCardListReaders(prv->mng, NULL, (LPTSTR)&rp, &len);
	if (ret != SCARD_S_SUCCESS || rp == NULL) {
		return B_CAS_CARD_ERROR_NO_SMART_CARD_READER;
	}
	readers = rp;
	BOOL find = FALSE;
...
#endif
	if (find) {
		readers = (LPTSTR)malloc((size_t)((namelen + 1)) * sizeof(TCHAR));
		if (readers == NULL) {
			return B_CAS_CARD_ERROR_NO_ENOUGH_MEMORY;
		}
		_tcscpy_s(readers, (size_t)namelen + 1, ini_name);
		len = namelen;
	}
	else {
		readers = (LPTSTR)malloc(((size_t)(len + 1)) * sizeof(TCHAR));
		if (readers == NULL) {
			return B_CAS_CARD_ERROR_NO_ENOUGH_MEMORY;
		}
		memcpy(readers, rp, len * sizeof(TCHAR));
		*(readers + len) = _T('\0');
	}
	alloc_len = len;
	alloc_len += 2;
	alloc_len = ROUNDUP(alloc_len, 0x7);	// 8の倍数で切り上げ

	m = (sizeof(TCHAR)* alloc_len) + (2*(size_t)B_CAS_BUFFER_MAX) + (sizeof(int64_t)*16) + (sizeof(B_CAS_PWR_ON_CTRL)*16);
	prv->pool = (uint8_t *)malloc(m);
	if(prv->pool == NULL){
		return B_CAS_CARD_ERROR_NO_ENOUGH_MEMORY;
	}
	prv->reader = (LPTSTR)(prv->pool);
	memcpy(prv->reader, readers, len * sizeof(TCHAR));
	memset(prv->reader+len, '\0', 2*sizeof(TCHAR));
	prv->sbuf = (uint8_t *)(prv->reader + alloc_len);
	...
	while (*(prv->reader) != _T('\0')) {
		if ((ret = connect_card(prv, prv->reader))) {
			break;
		}
	...
	free(readers);
	SCardFreeMemory(prv->mng, rp);

	return((ret) ? 0 : B_CAS_CARD_ERROR_ALL_READERS_CONNECTION_FAILED);
}
...
static int proc_emg_b_cas_card(void* bcas, uint8_t* src, int len)
{
	...
	slen = setup_emg_receive_command(prv->sbuf, src, len);
...
{
	int r;
	int length = (size_t)((*(src + 6)) << 8 | (*(src + 7)));

	r = sizeof(EMG_RECEIVE_CMD_HEADER);
	memcpy(dst + 0, EMG_RECEIVE_CMD_HEADER, r);
	dst[r] = (uint8_t)((len & 0xff) - 3);
	r += 1;

	memcpy(dst + r, src, 6);	/* card ID			*/
	r += 6;
	dst[r] = *(src + 8);		/* プロトコル番号	*/
	r += 1;
	dst[r] = *(src + 9);		/* 事業体識別		*/
	r += 1;
	dst[r] = *(src + 11);		/* メッセージ制御	*/
	r += 1;
	memcpy(dst + r, src + 12, (size_t)length - 4);	/* メッセージコード領域 */
	r += length - 4;
	dst[r] = 0;
	r += 1;
