...
#ifdef _WIN32
		strcpy_s(pattern,sizeof(pattern), name);
#else
		strcpy(pattern, name);
#endif
		return 0;
...
static int init_b_cas_card_with_name(void *bcas, const char * card_reader_name)
{
	...
	m = (sizeof(TCHAR)*len) + (2*(size_t)B_CAS_BUFFER_MAX) + (sizeof(int64_t)*16) + (sizeof(B_CAS_PWR_ON_CTRL)*16);
...
{
	int r;
	size_t length = (size_t)((*(src + 6)) << 8 | (*(src + 7)));
	...
	r += 1;
	memcpy(dst + r, src + 12, length - 4);	/* メッセージコード領域 */
	r += length - 4;
	dst[r] = 0;
	r += 1;
